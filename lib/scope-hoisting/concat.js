...
module.exports = (packager, ast) => {
  ...
  function replaceExportNode(module, originalName, path) {
    var _packager$findExportM = packager.findExportModule(
      module.id,
      originalName,
      replacements
    );
    ...
    VariableDeclarator: {
      exit(path) {
        ...
        if (t.isObjectPattern(id)) {
          ...
          try {
            for (
              var _iterator3 = path.get('id.properties')[Symbol.iterator](),
                _step3;
              ...
              var _packager$findExportM2 = packager.findExportModule(
                match[1],
                key.name,
                replacements
              );
        ...
        function replace(id, init, path) {
          ...
          try {
            for (
              var _iterator4 = binding.referencePaths[Symbol.iterator](),
                _step4;
    ...
    MemberExpression: {
      exit(path) {
        ...
        if (match) {
          ...
          var _packager$findExportM3 = packager.findExportModule(
            match[1],
            name,
            replacements
          );
